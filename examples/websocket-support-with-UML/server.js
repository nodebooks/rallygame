...
app.get('/', function(request, response) {
  umldata.push("Client-->Server: HTTP request /");
  response.sendFile(__dirname + '/public/index.html');
    umldata.push("Server-->Client: HTTP response /");

});

app.get('/getuml', function(request, response) {
  umldata.push("Client-->Server: HTTP request /umldata");
  response.send(JSON.stringify(umldata));
    umldata.push("Server-->Client: HTTP response /umldata");
...
server.on('connection', function(socket) {
  ...
  umldata.push("Server-->Client: HTTP connection response");
});

server.on('upgrade', function(request, socket, head){
  umldata.push("Client-->Server: HTTP connection upgrade request");
  ...
  umldata.push("Server-->Client: HTTP connection update response");


  // Bind 'data' event so we can communicate with the socket
  socket.on('data', function(chunk) {
    umldata.push("Client-->Server: Websocket data " + chunk.length + "bytes.");
    ...
    umldata.push("Server-->Client: Websocket data response " + respMessage.length + "bytes.");
  });

  socket.on('end', function() {
    umldata.push("Client-->Server: Websocket closed");
